// Migration script to add FOUNDER and COFOUNDER to application_type enum
import "dotenv/config";
import { pool } from "../server/db";

async function addApplicationTypes() {
  try {
    console.log("🔄 Adding FOUNDER and COFOUNDER to application_type enum...");
    
    // Check if enum values already exist
    const checkQuery = `
      SELECT unnest(enum_range(NULL::application_type))::text as value;
    `;
    ...
    if (!values.includes("FOUNDER")) {
      ...
      try {
        await pool.query(`ALTER TYPE application_type ADD VALUE 'FOUNDER'`);
    ...
    if (!values.includes("COFOUNDER")) {
      ...
      try {
        await pool.query(`ALTER TYPE application_type ADD VALUE 'COFOUNDER'`);
