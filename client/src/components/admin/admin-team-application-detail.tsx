...
export default function AdminTeamApplicationDetailPage() {
  const [, setLocation] = useLocation();
  const { user } = useAuth();
  const [match, params] = useRoute("/app/admin/team-applications/:id");
  const applicationId = params?.id;

  const { data: application, isLoading, error } = useQuery<TeamApplicationDetail>({
    queryKey: ["/api/admin/team-applications", applicationId],
    queryFn: async () => {
      const data = await apiRequest("GET", `/admin/team-applications/${applicationId}`);
      return data;
    },
        ...
        <div className="container mx-auto p-6">
          <Card>
            <CardContent className="pt-6">
              ...
              <Button onClick={() => setLocation("/app/admin/team-applications")} className="mt-4">
      ...
      <div className="container mx-auto p-6 space-y-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            <Button variant="ghost" size="icon" onClick={() => setLocation("/app/admin/team-applications")}>
