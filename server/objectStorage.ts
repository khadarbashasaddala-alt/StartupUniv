...
// Supports both Replit (development) and standard GCS credentials (production)
function createStorageClient(): Storage {
  // Check if running on Replit (development)
  if (process.env.REPL_ID && !process.env.GOOGLE_APPLICATION_CREDENTIALS) {
    return new Storage({
  credentials: {
    audience: "replit",
  ...
  // Production: Use standard GCS credentials
...
export class ObjectStorageService {
  ...
  async getSignedUrl(objectPath: string, expiresIn: number = 3600): Promise<string> {
    return this.getSignedDownloadURL(objectPath, expiresIn);
  }
...
async function signObjectURL({
  ...
  // If running on Replit, use sidecar endpoint
  if (process.env.REPL_ID && !process.env.GOOGLE_APPLICATION_CREDENTIALS) {
  const request = {
  ...
  // Production: Use GCS signed URL API
  const bucket = objectStorageClient.bucket(bucketName);
  const file = bucket.file(objectName);
